...
const EmployeeTasks = () => {
  ...
  // State to track approval status for each task
  ...
  // Define table headers for Employee Tasks Details (combined from EmployeeDetails and LeaveDecisions)
  const tableHead = ['#', 'Employee ID', 'Name', 'Department', 'Role', 'Leave Type', 'Start Date', 'End Date', 'Reason', 'Applied On', 'Status', 'Action'];
  const columnWidths = [50, 100, 150, 100, 100, 100, 100, 100, 150, 100, 100, 150];

  // Sample employee task entry
  const sampleEntry = [
    '1',                // #
    'EMP001',           // Employee ID
    'John Doe',         // Name
  ...
  // Combine sample entry into taskData
  const taskData = [sampleEntry];
      ...
      <View style={styles.innerContainer}>
        ...
        <View style={styles.navTabs}>
          ...
          <TouchableOpacity style={[styles.tab, styles.activeTab]}>
            <Text style={[styles.tabText, styles.activeTabText]}>Employee Tasks</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.contentContainer}>
              ...
              <Table borderStyle={{ borderWidth: 1, borderColor: '#ddd' }}>
                ...
                {taskData.length > 0 ? (
                  taskData.map((rowData, rowIndex) => {
                    const status = taskStatuses[rowIndex] || rowData[10]; // Use updated status if available
                    const isPending = status === 'Pending';
                    return (
                      <Row
                        key={rowIndex}
                        data={[
                          ...rowData.slice(0, 10), // #, Employee ID, Name, Department, Role, Leave Type, Start Date, End Date, Reason, Applied On
                  ...
                  <Row
                    data={[<Text style={styles.noDataText}>No employee tasks available</Text>]}
                    style={styles.tableRow}
                    textStyle={styles.tableText}
                    widthArr={[1150]} // Span the entire width
                  />
                )}
              </Table>
            </View>
          ...
          <Text style={styles.footerText}>
            Showing {taskData.length} to {taskData.length} of {taskData.length} rows
          </Text>
        </View>
